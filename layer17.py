...
(SimpleRequestHandler, setup_application) = (getattr(getattr(getattr(importlib.import_module('aiogram.webhook.aiohttp_server').SimpleRequestHandler, 'webhook'), 'aiohttp_server'), 'SimpleRequestHandler'), getattr(getattr(getattr(importlib.import_module('aiogram.webhook.aiohttp_server').setup_application, 'webhook'), 'aiohttp_server'), 'setup_application'))
(web,) = (getattr(importlib.import_module('aiohttp').web, 'web'),)
(backup_database,) = (getattr(importlib.import_module('backup').backup_database, 'backup_database'),)
(bot, dp) = (getattr(importlib.import_module('bot').bot, 'bot'), getattr(importlib.import_module('bot').dp, 'dp'))
(BACKUP_TIME, CRYPTO_BOT_ENABLE, DEV_MODE, LEGACY_ENABLE, ROBOKASSA_ENABLE, SUB_PATH, WEBAPP_HOST, WEBAPP_PORT, WEBHOOK_PATH, WEBHOOK_URL, YOOKASSA_ENABLE, YOOMONEY_ENABLE, PING_TIME) = (getattr(importlib.import_module('config').BACKUP_TIME, 'BACKUP_TIME'), getattr(importlib.import_module('config').CRYPTO_BOT_ENABLE, 'CRYPTO_BOT_ENABLE'), getattr(importlib.import_module('config').DEV_MODE, 'DEV_MODE'), getattr(importlib.import_module('config').LEGACY_ENABLE, 'LEGACY_ENABLE'), getattr(importlib.import_module('config').ROBOKASSA_ENABLE, 'ROBOKASSA_ENABLE'), getattr(importlib.import_module('config').SUB_PATH, 'SUB_PATH'), getattr(importlib.import_module('config').WEBAPP_HOST, 'WEBAPP_HOST'), getattr(importlib.import_module('config').WEBAPP_PORT, 'WEBAPP_PORT'), getattr(importlib.import_module('config').WEBHOOK_PATH, 'WEBHOOK_PATH'), getattr(importlib.import_module('config').WEBHOOK_URL, 'WEBHOOK_URL'), getattr(importlib.import_module('config').YOOKASSA_ENABLE, 'YOOKASSA_ENABLE'), getattr(importlib.import_module('config').YOOMONEY_ENABLE, 'YOOMONEY_ENABLE'), getattr(importlib.import_module('config').PING_TIME, 'PING_TIME'))
(init_db,) = (getattr(importlib.import_module('database').init_db, 'init_db'),)
(router,) = (getattr(importlib.import_module('handlers').router, 'router'),)
(handle_new_subscription, handle_old_subscription) = (getattr(getattr(getattr(importlib.import_module('handlers.keys.subscriptions').handle_new_subscription, 'keys'), 'subscriptions'), 'handle_new_subscription'), getattr(getattr(getattr(importlib.import_module('handlers.keys.subscriptions').handle_old_subscription, 'keys'), 'subscriptions'), 'handle_old_subscription'))
(periodic_notifications,) = (getattr(getattr(getattr(importlib.import_module('handlers.notifications.general_notifications').periodic_notifications, 'notifications'), 'general_notifications'), 'periodic_notifications'),)
(cryptobot_webhook,) = (getattr(getattr(getattr(importlib.import_module('handlers.payments.cryprobot_pay').cryptobot_webhook, 'payments'), 'cryprobot_pay'), 'cryptobot_webhook'),)
(validate_client_code,) = (getattr(getattr(getattr(importlib.import_module('handlers.payments.gift').validate_client_code, 'payments'), 'gift'), 'validate_client_code'),)
(robokassa_webhook,) = (getattr(getattr(getattr(importlib.import_module('handlers.payments.robokassa_pay').robokassa_webhook, 'payments'), 'robokassa_pay'), 'robokassa_webhook'),)
(MAIN_SECRET, yookassa_webhook) = (getattr(getattr(getattr(importlib.import_module('handlers.payments.yookassa_pay').MAIN_SECRET, 'payments'), 'yookassa_pay'), 'MAIN_SECRET'), getattr(getattr(getattr(importlib.import_module('handlers.payments.yookassa_pay').yookassa_webhook, 'payments'), 'yookassa_pay'), 'yookassa_webhook'))
(yoomoney_webhook,) = (getattr(getattr(getattr(importlib.import_module('handlers.payments.yoomoney_pay').yoomoney_webhook, 'payments'), 'yoomoney_pay'), 'yoomoney_webhook'),)
(logger,) = (getattr(importlib.import_module('logger').logger, 'logger'),)
(check_servers,) = (getattr(importlib.import_module('servers').check_servers, 'check_servers'),)
(os,) = (__import__('os'),)
(sys,) = (__import__('sys'),)
