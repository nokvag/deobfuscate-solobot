...
def install_cli():
    path_launcher = os.path.abspath('cli_launcher.py')
    python_exe = sys.executable
    search_dirs = ['/usr/local/bin', '/usr/bin', os.path.expanduser(
        '~/.local/bin')]
    for bin_dir in search_dirs:
        if os.path.isdir(bin_dir) and os.access(bin_dir, os.W_OK):
            script_file = os.path.join(bin_dir, 'solobot')
            break
    else:
        print(
            '❌ Не удалось найти подходящий каталог для установки команды `solobot`.'
            )
        return
...
async def main():
    ...
    if MAIN_SECRET != expected_secret:
        logger.error(
            'Нарушена целостность файлов! Обновитесь с полной заменой папки!')
        return
    ...
    if DEV_MODE:
        ...
        logger.info(
            'Polling остановлен в режиме разработки. Отмена фоновых задач...')
        ...
        app.router.add_get(f'{SUB_PATH}{{email}}/{{tg_id}}',
